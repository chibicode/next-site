...
import { GITHUB_URL, REPO_NAME, REPO_BRANCH } from '../github-constants';
...
export default async function markdownToHtml(filePath, md, customPlugin) {
  try {
    const repoUrl = `${GITHUB_URL}/${REPO_NAME}/blob/${REPO_BRANCH}`;
    // Init the processor with our custom plugin
    const processor = getProcessor().use(customPlugin, { filePath, repoUrl });
